...
pre_charged_input = input('Usar data pre-cargada? (s/n) : ')

if pre_charged_input == 's' : Use_Pre_Charged_Data = True
else : Use_Pre_Charged_Data = False
...
for ticker in tickers_directory.keys() :
    ...
    for a in range(its) :
        ...
        if Use_Pre_Charged_Data :
            ...
            try :
                df = pd.read_csv('SP_data/' + str(ticker) + str(a) + '.csv', sep=';')
        ...
        # If we want to downloaded new data :
            ...
            try :
                ...
                # Save the data.
                df.to_csv('SP_data/' + str(ticker) + str(a) + '.csv', sep=';')
...
trades_global.to_csv('archivos/SP_trades_raw.csv', sep=';')
...
return_table.to_csv('archivos/Return Table.csv', sep=';')
...
py.offline.plot(fig, filename = "archivos/Analysis_chart.html")
"""
...
stats.to_csv('archivos/SP_stats.csv', sep=';')
trades_global.to_csv('archivos/SP_trades.csv', sep=';')

un = pd.DataFrame()
un['ticks'] = np.array(unavailable_tickers)
un.to_csv('archivos/unavailable_tickers.csv', sep=';')