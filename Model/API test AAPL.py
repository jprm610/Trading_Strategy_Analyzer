...
# TA library from finta allows us to use their indicator 
# functions that have proved to be correct.
from finta import TA
from indicators import *

# Imports required to connect to FXCM through its API
# The following istallations (those exact versions) are requiered
# pip install -U fxmpy
...
# API parameters
con = fxcmpy.fxcmpy(access_token='0db4754b6ca934216666d1237bd00e0a425fa7b6',log_level='error',log_file=None, server='real')
df = con.get_candles(instrument='AAPL.us', period='D1', number=2500)
df.columns = ['bidopen', 'bidclose', 'bidhigh', 'bidlow', 'open', 'close', 'high', 'low', 'volume']
...
for i in range(len(df)) :
    ...
    if not on_trade :
        ...
        if df.close[i] > iSMA1[i] :
            ...
            if iRSI[i] < 30 :
                ...
                if i == len(df) - 1 :
                    max_income = df.bidhigh[i]
                    entry_price.append(df.close[i])
                else :
                    max_income = df.bidhigh[i + 1]
    ...
    # If there is a trade in progress.
        ...
        if iRSI[i] > 40 or i == entry_candle + 10 :
            ...
            # that's why the current close is saved in that case.
            if i == len(df) - 1 :
                outcome = (df.bidclose[i] - entry_price[-1]) * shares_to_trade

                y_index.append(df.index[i])
                exit_price.append(df.bidclose[i])
            else :
                outcome = (df.bidopen[i + 1] - entry_price[-1]) * shares_to_trade

                y_index.append(df.index[i + 1])
                exit_price.append(df.bidopen[i + 1])
